...
impl Puzzle for Puzzle02a {
    fn run() -> i32 {
        let puzzle_file = file::get_puzzle_input_file(1);
        Puzzle02a::run_for_file(puzzle_file)
    }
    ...
    fn run_for_file(file: File) -> i32 {
        ...
        for read_line in BufReader::new(file).lines() {
            ...
            let is_safe = Puzzle02a::is_safe(&level_list);
            if is_safe {
                safe_levels += 1;
            }
        }
...
mod tests {
    ...
    fn sample_tests_work() {
        ...
        let sample_0_is_safe = Puzzle02a::is_safe(&sample_vector_0);
        assert_eq!(sample_0_is_safe, true);

        let sample_vector_1 = vec![1, 2, 7, 8, 9];
        let sample_1_is_safe = Puzzle02a::is_safe(&sample_vector_1);
        assert_eq!(sample_1_is_safe, false);

        let sample_vector_2 = vec![9, 7, 6, 2, 1];
        let sample_2_is_safe = Puzzle02a::is_safe(&sample_vector_2);
        assert_eq!(sample_2_is_safe, false);

        let sample_vector_3 = vec![1, 3, 2, 4, 5];
        let sample_3_is_safe = Puzzle02a::is_safe(&sample_vector_3);
        assert_eq!(sample_3_is_safe, false);

        let sample_vector_4 = vec![8, 6, 4, 4, 1];
        let sample_4_is_safe = Puzzle02a::is_safe(&sample_vector_4);
        assert_eq!(sample_4_is_safe, false);

        let sample_vector_5 = vec![1, 3, 6, 7, 9];
        let sample_5_is_safe = Puzzle02a::is_safe(&sample_vector_5);
